...
if ! nc -z localhost 9093 2>/dev/null; then
    echo "❌ Kafka gateway is not running on localhost:9093"
    echo "Please start SeaweedMQ with Kafka gateway enabled first"
    exit 1
fi
echo "✅ Kafka gateway is running"
...
cd simple-publisher
go mod tidy
echo "✅ Publisher dependencies ready"

echo
echo "📤 Running publisher test..."
timeout 30s go run main.go || {
    echo "❌ Publisher test failed or timed out"
    exit 1
}
echo "✅ Publisher test completed"
...
cd ../simple-consumer
go mod tidy
echo "✅ Consumer dependencies ready"

echo
echo "📥 Testing consumer (will run for 10 seconds)..."
timeout 10s go run main.go || {
    if [ $? -eq 124 ]; then
        echo "✅ Consumer test completed (timed out as expected)"
    else
...
echo
echo "Summary:"
echo "- ✅ Topics with '_' prefix bypass schema validation"
echo "- ✅ Raw messages are stored as bytes in the 'value' field"
echo "- ✅ kafka-go client works with SeaweedMQ"
echo "- ✅ No schema validation errors for '_raw_messages' topic"
echo
