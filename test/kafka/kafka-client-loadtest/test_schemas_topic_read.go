...
func main() {
	fmt.Println("🧪 Testing _schemas Topic Read via Kafka Gateway")
	...
	// Get partition consumer for _schemas topic
	partitionConsumer, err := consumer.ConsumePartition("_schemas", 0, sarama.OffsetOldest)
	if err != nil {
		log.Fatalf("Failed to create partition consumer: %v", err)
	}
	defer partitionConsumer.Close()

	fmt.Println("📖 Reading messages from _schemas topic...")
