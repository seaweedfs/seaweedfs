...
func main() {
	fmt.Println("🧪 Complete Schema-Aware Workflow Test - Step by Step")
	fmt.Println("Testing the entire workflow: Schema Registry → Topic Creation → Message Production → Consumption")
	...
	topicName := "complete-workflow-test"
	...
	// Step 3: Verify schema can be retrieved
	fmt.Println("\n3️⃣  Step 3: Verify schema can be retrieved from Schema Registry")
	retrievedSchema, err := getSchemaByID(schemaID)
	if err != nil {
		log.Fatalf("❌ Failed to retrieve schema: %v", err)
	}
	fmt.Printf("✅ Schema retrieved successfully: %s\n", retrievedSchema[:100]+"...")

	// Step 4: Create topic (should now find schema and succeed)
	fmt.Println("\n4️⃣  Step 4: Create topic - Kafka Gateway should find schema in Registry")
	if err := createTopic(topicName); err != nil {
		log.Fatalf("❌ Failed to create topic: %v", err)
	}
	fmt.Printf("✅ Topic '%s' created successfully\n", topicName)

	// Step 5: Verify topic has schema in topic.conf
	fmt.Println("\n5️⃣  Step 5: Verify topic has schema persisted in topic.conf")
	...
	// Step 6: Produce schematized message
	fmt.Println("\n6️⃣  Step 6: Produce schematized message with Confluent Wire Format")
	if err := produceSchematizedMessage(topicName, schemaID); err != nil {
		log.Fatalf("❌ Failed to produce message: %v", err)
	}
	fmt.Printf("✅ Schematized message produced to topic '%s'\n", topicName)

	// Step 7: Consume and verify message
	fmt.Println("\n7️⃣  Step 7: Consume and verify message")
...
func getSchemaByID(schemaID int) (string, error) {
	url := fmt.Sprintf("http://schema-registry:8081/schemas/ids/%d", schemaID)
	...
	if resp.StatusCode != 200 {
		return "", fmt.Errorf("failed to get schema: status=%d", resp.StatusCode)
	}
