...
func main() {
	fmt.Println("🧪 Testing _schemas Topic Read from Offset 8 via Kafka Gateway")
	...
	// Get partition consumer for _schemas topic, starting from offset 8
	partitionConsumer, err := consumer.ConsumePartition("_schemas", 0, 8) // Start from offset 8
	if err != nil {
		log.Fatalf("Failed to create partition consumer: %v", err)
	}
	defer partitionConsumer.Close()

	fmt.Println("📖 Reading messages from _schemas topic starting from offset 8...")
