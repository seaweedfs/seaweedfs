...
func main() {
	fmt.Println("🧪 Testing _schemas Topic Read from Offset 6 via Kafka Gateway")
	...
	// Get partition consumer for _schemas topic, starting from offset 6
	partitionConsumer, err := consumer.ConsumePartition("_schemas", 0, 6) // Start from offset 6
	if err != nil {
		log.Fatalf("Failed to create partition consumer: %v", err)
	}
	defer partitionConsumer.Close()

	fmt.Println("📖 Reading messages from _schemas topic starting from offset 6...")
