...
func main() {
	fmt.Println("🧪 Testing _schemas Topic Read from Offset 12 (Final Timestamp Fix Verification)")
	...
	partitionConsumer, err := consumer.ConsumePartition("_schemas", 0, 12) // Start from offset 12
	if err != nil {
		log.Fatalf("Failed to create partition consumer: %v", err)
	}
	defer partitionConsumer.Close()

	fmt.Println("📖 Reading messages from _schemas topic starting from offset 12...")
