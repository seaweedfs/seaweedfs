...
func main() {
	fmt.Println("🔧 Testing Explicit Topic Creation with Schema")
	fmt.Println("Testing schema registration → explicit topic creation → produce → consume workflow")

	topic := fmt.Sprintf("test-explicit-%d", time.Now().Unix())
	...
	for {
		...
		case message := <-partitionConsumer.Messages():
			...
			if messageCount >= 3 {
				...
				fmt.Println("✅ Complete schema-aware workflow with explicit topic creation succeeded!")
