...
func main() {
	fmt.Println("Testing consumer group connection...")
	...
	// Create topic
	topicName := fmt.Sprintf("test-consumer-group-debug-%d", time.Now().Unix())
	...
	consumer, err := sarama.NewConsumerGroup([]string{"kafka-gateway:9093"}, "test-debug-group", consumerConfig)
	if err != nil {
...
func (h *DebugConsumerGroupHandler) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	fmt.Printf("Consuming from partition %d, offset %d to %d\n",
