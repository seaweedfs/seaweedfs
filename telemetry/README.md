...
```bash
# Enable telemetry in SeaweedFS master
weed master -telemetry.enabled=true -telemetry.url=http://localhost:8080/api/collect

# Or in server mode
weed server -telemetry.enabled=true -telemetry.url=http://localhost:8080/api/collect
```
