...
func CreateEntry(client SeaweedFilerClient, request *CreateEntryRequest) error {
	...
	if err != nil {
		log.V(2).Infof("create entry %s/%s %v: %v", request.Directory, request.Entry.Name, request.OExcl, err)
		return fmt.Errorf("CreateEntry: %v", err)
	}
	if resp.Error != "" {
		log.V(2).Infof("create entry %s/%s %v: %v", request.Directory, request.Entry.Name, request.OExcl, resp.Error)
...
func UpdateEntry(client SeaweedFilerClient, request *UpdateEntryRequest) error {
	...
	if err != nil {
		log.V(2).Infof("update entry %s/%s :%v", request.Directory, request.Entry.Name, err)
...
func LookupEntry(client SeaweedFilerClient, request *LookupDirectoryEntryRequest) (*LookupDirectoryEntryResponse, error) {
	...
	if err != nil {
		...
		log.V(0).Infof("read %s/%v: %v", request.Directory, request.Name, err)
