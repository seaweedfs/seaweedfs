...
func testEcRead(t *testing.T, large, small, datSize int64) {
	...
	err = generateEcFiles(baseFileName, int(small), large, small, ctx, nil)
...
func testDecodeDat(t *testing.T, datSize int64) {
	...
	err = generateEcFiles(baseFileName, 256*1024, ErasureCodingLargeBlockSize, ErasureCodingSmallBlockSize, ctx, nil)
	require.NoError(t, err, "EC encoding")
