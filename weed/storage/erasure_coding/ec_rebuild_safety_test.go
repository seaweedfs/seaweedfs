...
// TestRebuildEcFiles_ZeroSizedShardRegeneratedInPlace: a zero-sized shard file
// (residue of a failed operation, issue 10730) must be excluded from the
// rebuild inputs and regenerated byte-identical over the empty file, instead
// of failing the whole rebuild with an input size mismatch.
func TestRebuildEcFiles_ZeroSizedShardRegeneratedInPlace(t *testing.T) {
	dir := t.TempDir()
	base := filepath.Join(dir, "vol")
	ctx := NewDefaultECContext("", 0)
	writeRandomDat(t, base, 7000)
	...
	const emptied = 2
	want, err := os.ReadFile(base + ToExt(emptied))
	if err != nil {
		t.Fatalf("read shard %d: %v", emptied, err)
	}
	if err := os.Truncate(base+ToExt(emptied), 0); err != nil {
		t.Fatalf("truncate shard: %v", err)
	}

	generated, err := RebuildEcFiles(base, ctx, true)
	if err != nil {
		t.Fatalf("RebuildEcFiles with a zero-sized shard: %v", err)
	}
	found := false
	...
	if !found {
		t.Errorf("shard %d not reported as regenerated: %v", emptied, generated)
	}
	got, err := os.ReadFile(base + ToExt(emptied))
	if err != nil {
		t.Fatalf("read regenerated shard: %v", err)
	}
	if string(got) != string(want) {
		t.Errorf("regenerated shard %d differs from original (%d vs %d bytes)", emptied, len(got), len(want))
	}
}
