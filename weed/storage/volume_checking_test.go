...
func TestCheckIdxFile(t *testing.T) {
	testCases := []struct {
		name      string
		indexPath string
		...
		{
			name:      "healthy index",
			indexPath: "./idx/test_files/simple_index.idx",
		...
		},
		{
			...
			indexPath: "./idx/test_files/simple_index_damaged_1.idx",
			version:   needle.Version3,
			want:      120,
			wantErrs: []error{
		...
		},
		{
			...
			indexPath: "./idx/test_files/simple_index_damaged_2.idx",
			version:   needle.Version3,
			want:      2,
			wantErrs: []error{
		...
		},
		{
			...
			indexPath: "./idx/test_files/deleted_files.idx",
			version:   needle.Version3,
			want:      230,
			wantErrs:  []error{},
		},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, gotErrs := CheckIdxFile(icx, icxStat.Size(), tc.version)
