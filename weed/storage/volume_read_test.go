...
// scanRecorder records the offset of every record a scan visits. It fails the
// scan once it has seen more records than the file holds, so a scan that stops
// advancing ends instead of spinning.
...
// runWithTimeout returns fn's error, or errDidNotReturn if fn is still running
// after d, so a regression that hangs fails the test instead of the run.
...
// A corrupt .dat header with a negative size gives a record length,
// NeedleHeaderSize + NeedleBodyLength(size), that can be zero or negative.
// ScanVolumeFileFrom advanced by that length, so it re-read the same header
// forever or stepped back into the record before it. It must stop there with
// ErrorCorrupted. A negative size whose record length is still positive is
// stepped over as before.
