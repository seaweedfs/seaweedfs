...
func (v *Volume) load(alsoLoadIndex bool, createDatIfMissing bool, needleMapKind NeedleMapKind, preallocate int64) (err error) {
	...
	if v.HasRemoteFile() {
		...
		log.V(3).Infof("loading volume %d from remote %v", v.Id, v.volumeInfo)
		...
		if canWrite {
			...
			log.V(3).Infof("opening %s in READONLY mode", v.FileName(".dat"))
	...
	if alreadyHasSuperBlock {
		...
		log.V(3).Infof("readSuperBlock volume %d version %v", v.Id, v.SuperBlock.Version)
		if v.HasRemoteFile() {
			// maybe temporary network problem
			log.Errorf("readSuperBlock remote volume %d: %v", v.Id, err)
			err = nil
		}
	} else {
	...
	if err == nil && alsoLoadIndex {
		...
		if err := v.checkIdxFile(); err != nil {
			log.Fatalf("check volume idx file %s: %v", v.FileName(".idx"), err)
		}
		var indexFile *os.File
		if v.noWriteOrDelete {
			log.V(3).Infoln("open to read file", v.FileName(".idx"))
			...
			log.V(2).Infoln("open to write file", v.FileName(".idx"))
		...
		if !v.HasRemoteFile() {
			log.V(3).Infof("checking volume data integrity for volume %d", v.Id)
			if v.lastAppendAtNs, err = CheckVolumeDataIntegrity(v, indexFile); err != nil {
				v.noWriteOrDelete = true
				log.V(3).Infof("volumeDataIntegrityChecking failed %v", err)
			}
		}

		if v.noWriteOrDelete || v.noWriteCanDelete {
			if v.nm, err = NewSortedFileNeedleMap(v.IndexFileName(), indexFile); err != nil {
				log.V(3).Infof("loading sorted db %s error: %v", v.FileName(".sdx"), err)
			}
		} else {
			switch needleMapKind {
			case NeedleMapInMemory:
				if v.tmpNm != nil {
					log.V(3).Infof("updating memory compact index %s ", v.FileName(".idx"))
