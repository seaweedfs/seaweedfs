...
func ScanVolumeFileFrom(version needle.Version, datBackend backend.BackendStorageFile, offset int64, volumeFileScanner VolumeFileScanner) (err error) {
	...
	for n != nil {
		...
		// is zero or less. Advancing by it would re-read this header forever or
		// step back into the record before it. Fail rather than end quietly, so
		// callers such as compaction do not take a partial scan as complete.
