...
func rp(code string) *super_block.ReplicaPlacement {
	r, _ := super_block.NewReplicaPlacementFromString(code)
	return r
}
...
func TestIsGoodMove_NoReplication(t *testing.T) {
	...
	if !IsGoodMove(rp("000"), []types.ReplicaLocation{loc("dc1", "r1", "n1")}, "n1", loc("dc1", "r1", "n2")) {
...
func TestIsGoodMove_SameNode(t *testing.T) {
	...
	if IsGoodMove(rp("010"), existing, "n1", loc("dc1", "r2", "n2")) {
