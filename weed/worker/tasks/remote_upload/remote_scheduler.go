...
// RemoteUploadScheduler implements remote upload task scheduling
type RemoteUploadScheduler struct {
	maxConcurrent int
	enabled       bool
}
...
var (
	_ types.TaskScheduler = (*RemoteUploadScheduler)(nil)
)

// NewRemoteUploadScheduler creates a new remote upload scheduler
func NewRemoteUploadScheduler() *RemoteUploadScheduler {
	return &RemoteUploadScheduler{
		maxConcurrent: 1,
		enabled:       false,
	}
}
...
func (s *RemoteUploadScheduler) GetTaskType() types.TaskType {
	return types.TaskTypeRemoteUpload
}

// CanScheduleNow determines if a remote upload task can be scheduled now
func (s *RemoteUploadScheduler) CanScheduleNow(task *types.Task, runningTasks []*types.Task, availableWorkers []*types.Worker) bool {
	if !s.enabled {
		return false
	}
...
func (s *RemoteUploadScheduler) WasTaskRecentlyCompleted(task *types.Task, completedTasks []*types.Task, now time.Time) bool {
...
func (s *RemoteUploadScheduler) IsEnabled() bool {
	return s.enabled
}

// Configuration setters

func (s *RemoteUploadScheduler) SetEnabled(enabled bool) {
	s.enabled = enabled
}
