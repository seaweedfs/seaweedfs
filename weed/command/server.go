...
func init() {
	...
	serverOptions.v.diskIOProbe = cmdServer.Flag.Bool("volume.disk.io.probe", false, "enable disk IO latency probing to detect slow disks")
	serverOptions.v.diskIOTimeout = cmdServer.Flag.Duration("volume.disk.io.timeout", 2*time.Second, "maximum time allowed for disk IO probe before considering disk as failed")
	serverOptions.v.diskIOLatencyAlpha = cmdServer.Flag.Float64("volume.disk.io.latencyAlpha", 0.2, "EWMA smoothing factor for latency calculation (0-1), lower = smoother")
	serverOptions.v.diskIOMinSlowLatency = cmdServer.Flag.Duration("volume.disk.io.minSlowLatency", 50*time.Millisecond, "minimum latency threshold to consider as potential slow operation")
	serverOptions.v.diskIOSlowLatencyFactor = cmdServer.Flag.Float64("volume.disk.io.slowLatencyFactor", 3.0, "multiplier above average latency to flag as slow (current > avg * factor)")
	serverOptions.v.diskIOSlowStddevFactor = cmdServer.Flag.Float64("volume.disk.io.slowStddevFactor", 2.0, "multiplier above standard deviation to flag as slow (current > avg + stddev * factor)")
	serverOptions.v.diskIOMaxConsecutiveSlow = cmdServer.Flag.Int("volume.disk.io.maxConsecutiveSlow", 3, "number of consecutive slow operations required before reporting disk failure")
	serverOptions.v.diskIOMaxFailuresBeforeAlert = cmdServer.Flag.Int("volume.disk.io.maxFailuresBeforeAlert", 3, "number of IO failures tolerated before setting disk.Error alert")
