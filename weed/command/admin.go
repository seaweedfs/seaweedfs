...
func runAdmin(cmd *Command, args []string) bool {
	...
	err := startAdminServer(ctx, a, false)
	if err != nil {
...
func startAdminServer(ctx context.Context, options AdminOptions, disableUI bool) error {
	// Set Gin mode
	...
	adminHandlers.SetupRoutes(r, authRequired, *options.adminUser, *options.adminPassword, *options.readOnlyUser, *options.readOnlyPassword, disableUI)
