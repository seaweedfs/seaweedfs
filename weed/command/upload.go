...
func runUpload(cmd *Command, args []string) bool {
	...
	defaultReplication, err := readMasterConfiguration(grpcDialOptions, pb.ServerAddress(*upload.master))
	if err != nil {
	...
	if len(args) == 0 {
		...
		err = filepath.Walk(util.ResolvePath(*upload.dir), func(path string, info os.FileInfo, err error) error {
			if err == nil {
				if !info.IsDir() {
					...
					results, e := operation.SubmitFiles(func() pb.ServerAddress { return pb.ServerAddress(*upload.master) }, grpcDialOptions, parts, *upload.replication, *upload.collection, *upload.dataCenter, *upload.ttl, *upload.diskType, *upload.maxMB, *upload.usePublicUrl)
		...
		results, err := operation.SubmitFiles(func() pb.ServerAddress { return pb.ServerAddress(*upload.master) }, grpcDialOptions, parts, *upload.replication, *upload.collection, *upload.dataCenter, *upload.ttl, *upload.diskType, *upload.maxMB, *upload.usePublicUrl)
		if err != nil {
			fmt.Println(err.Error())
			return false
		}
...
func readMasterConfiguration(grpcDialOptions []grpc.DialOption, masterAddress pb.ServerAddress) (replication string, err error) {
	err = pb.WithMasterClient(false, masterAddress, grpcDialOptions, func(client master_pb.SeaweedClient) error {
