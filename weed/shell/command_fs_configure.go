...
func (c *commandFsConfigure) Do(args []string, commandEnv *CommandEnv, writer io.Writer) (err error) {
	...
	worm := fsConfigureCommand.Bool("worm", false, "wrom mode, if true, you can change a file only once, and then it is readonly and undeletable, see https://en.wikipedia.org/wiki/Write_once_read_many")
