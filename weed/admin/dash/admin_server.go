...
func NewAdminServer(masters string, filerGroup string, templateFS http.FileSystem, dataDir string, icebergPort, lancePort int, s3PublicEndpoint string) *AdminServer {
	...
	if err != nil {
		...
		if store := credentialManager.GetStore(); store != nil {
			...
			// Users/Groups pages can present a valid Bearer token: explicitly
			// configured, or auto-generated by a filer sharing this process
			// (weed server, weed mini) via WEED_JWT_FILER_SIGNING_KEY. A
			// filer running as a separate process needs the key set here too.
			if signer, ok := store.(interface {
				...
				if len(key) > 0 {
					...
					glog.Warningf("jwt.filer_signing.key is not set; IAM gRPC calls to a separate filer process will fail with Unauthenticated")
				}
			}
		}
	}
