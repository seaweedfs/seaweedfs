...
type BucketMetaData struct {
	...
	//By default, when another AWS account uploads an object to your S3 bucket,
	...
	if bucketMetadata == nil {
		...
		if err != nil {
			if err == filer_pb.ErrNotFound {
				// The bucket doesn't actually exist and is no longer loaded from the file
...
func (r *BucketRegistry) buildBucketMetadata(entry *filer_pb.Entry) *BucketMetaData {
	...
	bucketMetadata := &BucketMetaData{
		...
		//Default OwnershipBucketOwnerEnforced, which means Acl is disabled
		ObjectOwnership: &s3_constants.OwnershipBucketOwnerEnforced,

		//The default owner is the administrator. As long as the IAM Policy
		//verification is passed, the ACL can be accessed by default unless
		//an Account (not AccountAdmin) is custom configured.
		Owner: &s3.Owner{
