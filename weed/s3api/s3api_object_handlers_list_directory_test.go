...
func TestDirectoryListedAsCommonPrefix(t *testing.T) {
	t.Run("directory should appear as CommonPrefix with delimiter", func(t *testing.T) {
		s3a := &S3ApiServer{
		...
		_, err := s3a.doListFilerEntries(
			...
			cursor,
			"",  // marker
			"/", // delimiter - should create CommonPrefixes
			false,
			"xoa-bucket",
		...
		// The directory should be passed to the callback
		assert.Contains(t, seenDirs, "f2f1237f-0e69-4e0b-8f01-d4fa299787e1.vhd",
			"Directory should be passed to callback for CommonPrefix processing")
