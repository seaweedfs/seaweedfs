...
func TestListObjectsWithVersionedObjects(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := s3a.doListFilerEntries(context.Background(), filerClient, bucketPrefix[:len(bucketPrefix)-1], tt.prefix, cursor, "", tt.delimiter, false, tt.bucket, func(dir string, entry *filer_pb.Entry) {
				if cursor.maxKeys <= 0 {
					return
				}
...
func TestVersionedObjectsNoDuplication(t *testing.T) {
	...
	contents := []ListEntry{}
	_, err := s3a.doListFilerEntries(context.Background(), filerClient, "/buckets/test-bucket", "", cursor, "", "", false, "test-bucket", func(dir string, entry *filer_pb.Entry) {
		if cursor.maxKeys <= 0 {
			return
		}
...
func TestVersionsDirectoryNotTraversed(t *testing.T) {
	...
	cursor := &ListingCursor{maxKeys: uint16(1000)}
	contents := []ListEntry{}
	_, err := s3a.doListFilerEntries(context.Background(), customClient, "/buckets/test-bucket", "", cursor, "", "", false, "test-bucket", func(dir string, entry *filer_pb.Entry) {
		if cursor.maxKeys <= 0 {
			return
		}
		contents = append(contents, ListEntry{Key: entry.Name})
