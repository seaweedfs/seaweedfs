...
// AuthSignatureOnly validates the request signature but skips authorization.
...
// Authenticate verifies the request credentials and returns the identity
func (iam *IdentityAccessManagement) Authenticate(r *http.Request) (*Identity, s3err.ErrorCode) {
...
func (iam *IdentityAccessManagement) authRequest(r *http.Request, action Action) (*Identity, s3err.ErrorCode) {
	identity, s3Err := iam.Authenticate(r)
...
// GetIAMIntegration returns the IAM integration
...
func (iam *IdentityAccessManagement) authorizeWithIAM(r *http.Request, identity *Identity, action Action, bucket string, object string) s3err.ErrorCode {
	...
	// Extract attached policies from identity actions
	// Legacy identities store policies as actions prefixed with "policy:"
	for _, action := range identity.Actions {
		actionStr := string(action)
		if strings.HasPrefix(actionStr, "policy:") {
			policyName := strings.TrimPrefix(actionStr, "policy:")
			if policyName != "" {
				iamIdentity.AttachedPolicies = append(iamIdentity.AttachedPolicies, policyName)
			}
		}
	}
	...
	if sessionToken != "" && principal != "" {
		...
		// For V4 auth, the session token comes from the X-Amz-Security-Token header
		iamIdentity.SessionToken = r.Header.Get("X-Amz-Security-Token")
...
// VerifyActionPermission checks if the identity has permission to perform the specified action
func (iam *IdentityAccessManagement) VerifyActionPermission(r *http.Request, identity *Identity, action Action, bucket, object string) s3err.ErrorCode {
	...
	// For STS actions on a resource (like AssumeRole), we check if the user has permission policies
	// The resource passed here is typically partial, so we use canDo's logic or enhance it.
	if identity.canDo(action, bucket, object) {
		return s3err.ErrNone
	}

	// If standard simple permission check fails, return AccessDenied
	return s3err.ErrAccessDenied
}
