...
func NewFileReader(ctx context.Context, filerClient filer_pb.FilerClient, entry *filer_pb.Entry) io.Reader {
	if len(entry.Content) > 0 {
		return bytes.NewReader(entry.Content)
	}
	return NewChunkStreamReader(ctx, filerClient, entry.GetChunks())
}

type DoStreamContent func(writer io.Writer) error

func PrepareStreamContent(ctx context.Context, masterClient wdclient.HasLookupFileIdFunction, jwtFunc VolumeServerJwtFunction, chunks []*filer_pb.FileChunk, offset int64, size int64) (DoStreamContent, error) {
	return PrepareStreamContentWithThrottler(ctx, masterClient, jwtFunc, chunks, offset, size, 0)
}
...
func StreamContent(masterClient wdclient.HasLookupFileIdFunction, writer io.Writer, chunks []*filer_pb.FileChunk, offset int64, size int64) error {
	streamFn, err := PrepareStreamContent(context.Background(), masterClient, noJwtFunc, chunks, offset, size)
	if err != nil {
		return err
	}
...
func NewChunkStreamReader(ctx context.Context, filerClient filer_pb.FilerClient, chunks []*filer_pb.FileChunk) *ChunkStreamReader {
	...
	for p != nil {
		...
		if insideChunk(offset, chunk) {
			if c.isBufferEmpty() || c.bufferOffset != chunk.ViewOffset {
				c.chunkView = p
				return c.fetchChunkToBuffer(context.Background(), chunk)
			}
		}
...
func (c *ChunkStreamReader) fetchChunkToBuffer(ctx context.Context, chunkView *ChunkView) error {
	...
	for _, urlString := range urlStrings {
		shouldRetry, err = util_http.ReadUrlAsStream(ctx, urlString+"?readDeleted=true", chunkView.CipherKey, chunkView.IsGzipped, chunkView.IsFullChunk(), chunkView.OffsetInChunk, int(chunkView.ViewSize), func(data []byte) {
			buffer.Write(data)
		})
		if !shouldRetry {
			break
