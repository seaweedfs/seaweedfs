...
func BenchmarkManifestResolutionRepeatedOpen(b *testing.B) {
	for _, delay := range []time.Duration{0, 5 * time.Millisecond, 20 * time.Millisecond} {
		b.Run(fmt.Sprintf("latency=%s", delay), func(b *testing.B) {
			...
			_, err := resolveOneChunkManifest(context.Background(), lookup, chunk, nil, mountChunkManifestCache)
			if err != nil {
				b.Fatal(err)
			}
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				if _, err := resolveOneChunkManifest(context.Background(), lookup, chunk, nil, mountChunkManifestCache); err != nil {
					b.Fatal(err)
				}
			}
			b.StopTimer()
