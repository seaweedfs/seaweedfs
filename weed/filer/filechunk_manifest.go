...
func fetchChunkRange(buffer []byte, lookupFileIdFn wdclient.LookupFileIdFunctionType, fileId string, cipherKey []byte, isGzipped bool, offset int64) (int, error) {
	...
	return util_http.RetriedFetchChunkData(util_http.GetGlobalHttpClient(), buffer, urlStrings, cipherKey, isGzipped, false, offset)
}
...
func retriedStreamFetchChunkData(writer io.Writer, urlStrings []string, jwt string, cipherKey []byte, isGzipped bool, isFullChunk bool, offset int64, size int) (err error) {
	...
	for waitTime := time.Second; waitTime < util.RetryWaitTime; waitTime += waitTime / 2 {
		for _, urlString := range urlStrings {
			...
			var writeErr error
			shouldRetry, err = util_http.ReadUrlAsStreamAuthenticated(util_http.GetGlobalHttpClient(), urlString+"?readDeleted=true", jwt, cipherKey, isGzipped, isFullChunk, offset, size, func(data []byte) {
