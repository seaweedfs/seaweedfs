...
func (t *Topology) batchVacuumVolumeCheck(grpcDialOptions []grpc.DialOption, vid needle.VolumeId,
	...
	for index, dn := range locationlist.list {
		go func(index int, url pb.ServerAddress, vid needle.VolumeId) {
			err := operation.WithVolumeServerClient(true, url, grpcDialOptions, func(volumeServerClient volume_server_pb.VolumeServerClient) error {
...
func (t *Topology) batchVacuumVolumeCompact(grpcDialOptions []grpc.DialOption, vl *VolumeLayout, vid needle.VolumeId,
...
func (t *Topology) batchVacuumVolumeCommit(grpcDialOptions []grpc.DialOption, vl *VolumeLayout, vid needle.VolumeId, locationlist *VolumeLocationList) bool {
	isCommitSuccess := true
	isReadOnly := false
	for _, dn := range locationlist.list {
		glog.V(0).Infoln("Start Committing vacuum", vid, "on", dn.Url())
		err := operation.WithVolumeServerClient(true, dn.ServerAddress(), grpcDialOptions, func(volumeServerClient volume_server_pb.VolumeServerClient) error {
...
func (t *Topology) batchVacuumVolumeCleanup(grpcDialOptions []grpc.DialOption, vl *VolumeLayout, vid needle.VolumeId, locationlist *VolumeLocationList) {
	for _, dn := range locationlist.list {
...
func (t *Topology) Vacuum(grpcDialOptions []grpc.DialOption, garbageThreshold float64, preallocate int64) {
	...
	for _, col := range t.collectionMap.Items() {
		...
		for _, vl := range c.storageType2VolumeLayout.Items() {
			if vl != nil {
				...
				t.vacuumOneVolumeLayout(grpcDialOptions, volumeLayout, c, garbageThreshold, preallocate)
			}
		}
	}
}

func (t *Topology) vacuumOneVolumeLayout(grpcDialOptions []grpc.DialOption, volumeLayout *VolumeLayout, c *Collection, garbageThreshold float64, preallocate int64) {
	...
	for vid, locationList := range tmpMap {
		...
		if vacuumLocationList, needVacuum := t.batchVacuumVolumeCheck(grpcDialOptions, vid, locationList, garbageThreshold); needVacuum {
			if t.batchVacuumVolumeCompact(grpcDialOptions, volumeLayout, vid, vacuumLocationList, preallocate) {
				t.batchVacuumVolumeCommit(grpcDialOptions, volumeLayout, vid, vacuumLocationList)
			} else {
				t.batchVacuumVolumeCleanup(grpcDialOptions, volumeLayout, vid, vacuumLocationList)
			}
		}
	}
}
