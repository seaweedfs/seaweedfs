...
`
	...
	for i := 0; i < 6; i++ {
		...
		recovered := vl.UpdateVolumeSize(1, 4000, 0, false)
		if recovered {
...
`
	...
	for i := 0; i < 5; i++ {
		advanceSizeTrackingClock(vl, 1, 10*time.Second)
		if vl.UpdateVolumeSize(1, 10500, 0, false) {
...
`
	...
	advanceCycle()
	vl.UpdateVolumeSize(1, 5000, 0, false)
	...
	advanceCycle()
	vl.UpdateVolumeSize(1, 5500, 0, false)
...
`
	...
	vl.UpdateVolumeSize(1, 3000, 0, false)
	vl.UpdateVolumeSize(1, 3000, 0, false) // second replica, same size — should be a no-op
...
`
	...
	vl.UpdateVolumeSize(1, 1000, 0, false)
