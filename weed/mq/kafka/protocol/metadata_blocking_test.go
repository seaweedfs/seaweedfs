...
// TestMetadataRequestBlocking reproduces the issue where Metadata requests hang
// when the backend (broker/filer) ListTopics call blocks indefinitely.
// This test directly calls the handleMetadata function with a blocking mock handler.
func TestMetadataRequestBlocking(t *testing.T) {
	...
	case <-time.After(3 * time.Second):
		...
		t.Logf("  Solution needed: Add timeout context to ListTopics() call")
		t.Errorf("CRITICAL BUG: handleMetadata blocks when backend is slow - should return error response within reasonable time")
	}
}

// TestMetadataRequestWithTimeoutFix tests that Metadata requests properly timeout
// This test documents the EXPECTED behavior after the fix
func TestMetadataRequestWithTimeoutFix(t *testing.T) {
	t.Skip("This test demonstrates the expected behavior after implementing timeout fix")
	...
	handler := &Handler{
		seaweedMQHandler: &TimeoutAwareMockHandler{
			...
			blockDuration: 10 * time.Second, // Backend is slow
		},
	}
	...
	if err != nil {
		...
		// Check that it's an error response (has error code)
		if len(response) >= 2 {
			// Note: Response format varies by version, but should indicate error
			t.Logf("Response returned - backend timeout handled gracefully")
		}
	} else {
