...
func TestExtractAllRecords_RealKafkaFormat(t *testing.T) {
	...
	// headersCount (unsigned varint - 0)
	record = append(record, 0)
	
	// Prepend record length as varint
	recordLength := len(record)
	recordWithLength := make([]byte, 0, recordLength+5)
	recordWithLength = append(recordWithLength, byte(recordLength))
