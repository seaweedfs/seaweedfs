...
func (u *fakeChunkUploader) UploadWithRetry(_ filer_pb.FilerClient, assignRequest *filer_pb.AssignVolumeRequest, uploadOption *operation.UploadOption, reader io.Reader) (string, *operation.UploadResult, error, []byte) {
	...
	// Mirror operation.Uploader's URL resolution so the recorded URL reflects
	// what SaveGatewayDataAsChunk wired into the upload option (e.g. filerProxy).
	genUploadUrl := uploadOption.GenUploadUrl
	if genUploadUrl == nil {
		genUploadUrl = func(host, fileId string) string { return fmt.Sprintf("http://%s/%s", host, fileId) }
	}

	u.calls = append(u.calls, fakeChunkUploadCall{
		assignRequest: assignClone,
		uploadOption:  optionClone,
		uploadURL:     genUploadUrl("volume.example:8080", fileID),
