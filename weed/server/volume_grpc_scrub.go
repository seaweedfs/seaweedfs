...
func (vs *VolumeServer) ScrubVolume(ctx context.Context, req *volume_server_pb.ScrubVolumeRequest) (*volume_server_pb.ScrubVolumeResponse, error) {
	...
	for _, vid := range vids {
		...
		case volume_server_pb.VolumeScrubMode_NEEDLES:
			files, serrs = scrubVolumeNeedles(ctx, v)
...
func scrubVolumeNeedles(ctx context.Context, v *storage.Volume) (uint64, []error) {
	return 0, []error{fmt.Errorf("scrubVolumeNeedles(): not implemented")}
}
...
func (vs *VolumeServer) ScrubEcVolume(ctx context.Context, req *volume_server_pb.ScrubEcVolumeRequest) (*volume_server_pb.ScrubEcVolumeResponse, error) {
	...
	for _, vid := range vids {
		...
		switch m := req.GetMode(); m {
		case volume_server_pb.VolumeScrubMode_NEEDLES:
			files, serrs = scrubEcVolumeNeedles(v)
		case volume_server_pb.VolumeScrubMode_FULL:
...
func scrubEcVolumeNeedles(ecv *erasure_coding.EcVolume) (uint64, []error) {
	return 0, []error{fmt.Errorf("scrubEcVolumeNeedles(): not implemented")}
}
