...
func (fs *FilerServer) saveMetaData(ctx context.Context, r *http.Request, fileName string, contentType string, so *operation.StorageOption, md5bytes []byte, fileChunks []*filer_pb.FileChunk, chunkOffset int64, content []byte) (filerResult *FilerPostResult, replyerr error) {
	...
	if dbErr := fs.filer.CreateEntry(ctx, entry, false, false, nil, skipCheckParentDirEntry(r), so.LongerFileName); dbErr != nil {
		replyerr = dbErr
...
func (fs *FilerServer) mkdir(ctx context.Context, w http.ResponseWriter, r *http.Request, so *operation.StorageOption) (filerResult *FilerPostResult, replyerr error) {
	...
	if dbErr := fs.filer.CreateEntry(ctx, entry, false, false, nil, false, so.LongerFileName); dbErr != nil {
		replyerr = dbErr
		filerResult.Error = dbErr.Error()
