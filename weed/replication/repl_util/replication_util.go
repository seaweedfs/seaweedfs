...
func CopyFromChunkViews(chunkViews *filer.IntervalList[*filer.ChunkView], filerSource *source.FilerSource, writeFunc func(data []byte) error) error {
	...
	for x := chunkViews.Front(); x != nil; x = x.Next {
		...
		for _, fileUrl := range fileUrls {
			shouldRetry, err = http_unknown.ReadUrlAsStream(fileUrl, chunk.CipherKey, chunk.IsGzipped, chunk.IsFullChunk(), chunk.OffsetInChunk, int(chunk.ViewSize), func(data []byte) {
